"OneKey QQPet 1.2.1 pure business extraction.";
...
const DEFAULT_CONFIG_VALUES = {
  ...
  schoolEnabled: !0,
  schoolEncouragementEnabled: !0,
  schoolAttribute: "physical",
  schoolRotationEnabled: !1,
  ...
  adventureEnabled: !0,
  quickAdventureEnabled: !1,
  adventurePreviewRefreshLimit: 20,
  adventureMoneyBagStopEnabled: !1,
...
function normalizeConfig(input) {
  ...
  const ref = record;
  if (ref.visitDelayMinSeconds === undefined && ref.visitDelayMinMinutes !== undefined) {
    ref.visitDelayMinSeconds = ref.visitDelayMinMinutes;
  }
  if (ref.visitDelayMaxSeconds === undefined && ref.visitDelayMaxMinutes !== undefined) {
    ref.visitDelayMaxSeconds = ref.visitDelayMaxMinutes;
  }
  for (const temp of Object.keys(DEFAULT_CONFIG)) {
    const member = ref[temp];
    if (typeof DEFAULT_CONFIG[temp] == typeof member) {
      temp_3[temp] = member;
    }
  }
  temp_3.visitStompEnabled = !1;
  for (const temp_2 of NUMERIC_CONFIG_KEYS) {
    const number = Number(ref[temp_2] ?? temp_3[temp_2]);
  ...
  if (!["rest", "work", "school", "adventure"].includes(temp_3.fatigue8HourAction)) {
    temp_3.fatigue8HourAction = "rest";
  }
  if (!["rest", "work", "school", "adventure"].includes(temp_3.fatigue12HourAction)) {
  ...
  temp_3.pkDelayMinSeconds = Math.trunc(temp_3.pkDelayMinSeconds);
  temp_3.pkDelayMaxSeconds = Math.max(temp_3.pkDelayMinSeconds, Math.trunc(temp_3.pkDelayMaxSeconds));
...
function getFloatField(fields, fieldNumber, input3 = 0) {
  ...
  return new DataView(member.buffer, member.byteOffset, 4).getFloat32(0, !0);
}
...
const packetKey = ([packet, packet_2, packet_3]) => packet + ":" + packet_2 + ":" + packet_3;
...
function parseRewardAmount(rewardText) {
  const text = "(\\d+(?:\\.\\d+)?)";
  const member = rewardText.match(new RegExp("(?:总收益|合计|总计)\\D*" + text))?.[1];
  const member_2 = rewardText.match(new RegExp("金币\\D*" + text))?.[1];
  const member_3 = rewardText.match(new RegExp(text))?.[1];
  return Number(member ?? member_2 ?? member_3 ?? 0);
}
...
function parseMedal(bytes, acquired = !0, equipped) {
...
function parseDurationSeconds(durationText) {
  const number_2 = Number(durationText.match(/(\d+)\s*小时/)?.[1] ?? 0);
  const number_3 = Number(durationText.match(/(\d+)\s*分钟/)?.[1] ?? 0);
  const number = Number(durationText.match(/(\d+)\s*秒/)?.[1] ?? 0);
  return number_2 * 3600 + number_3 * 60 + number;
}
...
class QQPetApi {
  ...
  async sendPacket(packet, body) {
    ...
    const temp_5 = packetKey(packet);
    const temp_3 = READ_ONLY_PACKET_KEYS.has(temp_5) ? undefined : "未知协议写请求";
    const hex = bytesToHex(body);
    let temp_4;
    if (temp_3) {
      temp_4 = await this.executeMutation(temp_3, "unregistered.operation", hex, {
        command: commandName,
        dataHex: hex
      });
    } else {
      try {
        ...
        temp_4 = await this.ctx.actions.call("send_packet", packetRequest, this.ctx.adapterName, this.ctx.pluginManager.config);
      } catch (temp) {
  ...
  async sendProtectedOidb(operation, actionLabel, body) {
    const temp_3 = await this.executeMutation(actionLabel, operation, bytesToHex(body), undefined);
    const packetHex = extractPacketHex(temp_3);
    if (!packetHex) {
      throw new QQPetError(actionLabel + "返回空响应或未知响应结构");
    }
    let temp_2;
    try {
      temp_2 = hexToBytes(packetHex);
    } catch (temp) {
      throw new QQPetError(actionLabel + "响应无法解析：" + String(temp));
    }
    const fields = parseProtobufFields(temp_2);
    const varint_2 = getVarintField(fields, 1);
    const varint = getVarintField(fields, 2);
    const temp_5 = varint_2 > 0 && fields.has(4);
    const temp_4 = temp_5 ? getVarintField(fields, 3) : 0;
    if (temp_4) {
      throw new QQPetError(actionLabel + " OIDB errorCode=" + temp_4);
    }
    return {
      command: varint_2,
      subCommand: varint,
      errorCode: temp_4,
      body: temp_5 ? getBytesField(fields, 4) : temp_2,
      raw: temp_2
    };
  }
  async queryOwnPetProfile() {
    ...
    if (trimmed_2) {
      const fieldBytes = getBytesField(temp_7, 13);
      const temp_4 = fieldBytes.length ? parseProtobufFields(fieldBytes) : new Map();
      const fieldBytes_2 = getBytesField(temp_7, 14);
      const temp_3 = fieldBytes_2.length ? parseProtobufFields(fieldBytes_2) : new Map();
      const varint_2 = getVarintField(temp_7, 6);
      const mappedItems = getProtobufFields(temp_3, 1).filter(temp => temp.wireType === 2).map(temp_2 => parseMedal(temp_2.value, !0, !0));
      return {
        petId: trimmed_2,
    ...
    try {
      const temp_6 = await this.sendOidb(READ_PACKETS.ownPetCache, new Uint8Array());
      const fieldBytes_3 = getBytesField(parseProtobufFields(temp_6.body), 1);
      const temp_5 = fieldBytes_3.length ? parseProtobufFields(fieldBytes_3) : new Map();
      const trimmed = getStringField(temp_5, 101).trim();
      if (trimmed) {
        return {
          petId: trimmed,
          name: getStringField(temp_5, 1).trim(),
          birthdayAt: 0,
          gender: "未知",
          species: "",
          personality: "",
          level: 0,
          avatarUrl: getStringField(temp_5, 3).trim(),
          fullAvatarUrl: getStringField(temp_5, 3).trim(),
          personalityUrl: "",
          medals: [],
          currentExperience: 0,
          levelExperience: 0,
  ...
  async queryOutdoorRecords(limit = 20, offset = 0) {
    const bytes = concatBytes(encodeVarintField(1, Math.max(0, Math.trunc(offset))), encodeVarintField(2, Math.max(1, Math.min(50, Math.trunc(limit)))), encodeStringField(3, this.petId), encodeVarintField(100, 2));
    const fields_4 = parseProtobufFields((await this.sendOidb(READ_PACKETS.outdoorHistory, bytes)).body);
    const varint_3 = getVarintField(fields_4, 4);
    return getProtobufFields(fields_4, 1).filter(temp => temp.wireType === 2).flatMap(temp_4 => {
      const fields_2 = parseProtobufFields(temp_4.value);
      const trimmed = getStringField(fields_2, 1).trim();
      if (!trimmed) {
        return [];
      }
      const varint_2 = getVarintField(fields_2, 2);
      const fields_3 = parseProtobufFields(getBytesField(fields_2, 6));
      const mappedItems = getProtobufFields(fields_3, 1).filter(temp_2 => temp_2.wireType === 2).map(temp_3 => {
        ...
        return {
          ...
          type: varint === 1 ? 2 : varint === 2 ? 3 : varint,
      ...
      return [{
        storyId: trimmed,
        timestamp: varint_2,
        eventType: getVarintField(fields_2, 7),
        grade: getVarintField(fields_2, 4),
        title: getStringField(fields_2, 3).trim(),
        detail: getStringField(fields_2, 5).trim(),
        results: mappedItems,
        unread: varint_2 > varint_3,
        outdoorVersion: getVarintField(fields_2, 8)
      }];
    });
  }
  async queryValues() {
  ...
  async queryOtherPet(uin, kind) {
    ...
    if (this.otherPetMobileAvailable === !1) {
    ...
    try {
      ...
      this.otherPetMobileAvailable = !0;
      const fieldBytes_2 = getBytesField(fields, 1);
      return handler(fieldBytes_2.length ? parseProtobufFields(fieldBytes_2) : fields);
    } catch (temp_4) {
      const temp_3 = temp_4 instanceof Error ? temp_4.message : String(temp_4);
      if (/用户没有宠物|尚未创建.*宠物|未创建.*宠物/.test(temp_3)) {
        this.otherPetMobileAvailable = !0;
        return null;
      }
      throw /不支持.*好友宠物|请升级客户端版本|action.*(?:不存在|not found|unsupported)|返回空响应或未知响应结构|unknown command/i.test(temp_3) ? (this.otherPetMobileAvailable = !1, new QQPetError("当前 QQ/OneBot 运行时不支持安卓端的好友宠物详情接口：" + temp_3, "other_pet_unsupported")) : new QQPetError("查询好友宠物失败：" + temp_3, "other_pet_query_failed");
    }
  }
  ...
  async queryPetFriendRecords(page, directoryType = PET_DIRECTORY_TYPES.friends) {
    ...
    for (let number = 0; number < 100; number += 1) {
      ...
      for (const temp_3 of getProtobufFields(fields_5, 1).filter(temp => temp.wireType === 2)) {
        ...
        const fields_4 = parseProtobufFields(getBytesField(fields_2, 2));
        const trimmed = (getStringField(fields, 8) || getStringField(fields, 101)).trim();
        const text = String(getVarintField(fields_4, 1));
        if (!trimmed || !/^[1-9]\d{4,11}$/.test(text)) {
          continue;
        }
        const fields_3 = parseProtobufFields(getBytesField(fields, 13));
        list.push({
          target: {
            uin: text,
            petId: trimmed,
            name: (getStringField(fields, 1) || getStringField(fields_4, 2)).trim(),
            level: getVarintField(fields_3, 1),
  ...
  async queryPetFriendDirectory() {
    const list = await this.queryPetFriendRecords(!0);
    const mappedItems_2 = list.map(temp => {
    ...
    const mappedItems = list.filter(temp_2 => temp_2.isFriend).map(temp_3 => {
    ...
    const record = {
      petDirectory: mappedItems_2,
      workHireFriends: mappedItems
    };
    return record;
  }
  ...
  async queryPetAlsoPlayingDirectory() {
    return (await this.queryPetFriendRecords(!0, PET_DIRECTORY_TYPES.alsoPlaying)).filter(temp => !temp.isFriend).map(temp_2 => {
  ...
  async visitOther(uin) {
    ...
    await this.sendProtectedOidb("pet.report-event", "走访宠物", bytes_2);
  }
  ...
  async queryFoodInventory() {
    ...
    const varint = getVarintField(fields_2, 1);
    const varint_2 = getVarintField(fields_2, 2);
    const handler = (input, input2) => list.find(temp_2 => temp_2.name.includes(input))?.balance ?? (list.length ? 0 : input2);
    return {
      biscuits: handler("饼干", varint),
      shrimp: handler("虾仁", varint_2),
      allowanceRemaining: varint,
      allowanceMax: varint_2,
      items: list
    };
  }
  async buyFood(count) {
  ...
  async startWork(careerType = 0, jobSubEvent = 0, hire) {
    const temp_3 = hire?.uin.trim() ?? "";
    const temp_2 = hire?.petId.trim() ?? "";
    if (!!temp_3 != !!temp_2) {
      throw new QQPetError("雇佣好友时必须同时提供好友账号和宠物 ID");
    }
    const catalogItem = await this.selectWorkJob(careerType, jobSubEvent, temp_2);
    const bytes = concatBytes(encodeVarintField(1, 6400), encodeStringField(2, this.petId), encodeStringField(3, ""), temp_3 ? encodeBytesField(4, concatBytes(encodeStringField(1, temp_3), encodeStringField(2, temp_2))) : new Uint8Array(), encodeStringField(6, catalogItem.name), encodeVarintField(7, catalogItem.subEventType), encodeVarintField(100, 2));
    const temp = await this.sendProtectedOidb("pet.story-start", "开始任务", bytes);
    return {
      item: catalogItem,
      storyId: getStringField(parseProtobufFields(temp.body), 1),
  ...
  async startAdventure(optionName = "", quickMode = !1, previewRefreshLimit = 30) {
    let catalogItem;
    let number = 0;
    for (; number < 1; number += 1) {
      const list_2 = (await this.queryAdventureOptions()).filter(temp => temp.canDo && temp.name);
      const list = optionName ? list_2.filter(temp_2 => temp_2.name === optionName) : list_2;
      catalogItem = quickMode ? list.find(adventureHasMoneyBag) : list[0];
      if (catalogItem) {
        break;
      }
    }
  ...
  async queryStory() {
    ...
    const fieldText = getStringField(fields, 2);
    const varint_2 = getVarintField(temp, 2);
    const varint = getVarintField(temp, 3);
    return {
      storyId: fieldText,
      stateCode: getVarintField(temp, 1),
      remainingSeconds: varint_2,
      durationSeconds: varint,
      startedAt: getVarintField(temp, 4),
      recallable: !!getVarintField(temp, 5),
      finished: !!fieldText && !!(varint > 0) && !!(varint_2 <= 0)
    };
  }
...
function selectSchoolOrWork(config, values, counts) {
  ...
  const ref = record;
  return (config.taskPriority === "work" ? ["work", "school"] : ["school", "work"]).find(temp => ref[temp]) ?? null;
}
...
function updateAdventureMoneyBagStreak(previousStreak, beforeGold, afterGold, limit) {
  ...
  const temp_2 = matches ? 0 : Math.max(0, Math.trunc(previousStreak)) + 1;
  const temp = Math.max(1, Math.trunc(limit));
  return {
    dropped: matches,
    goldGain: matches ? Math.max(1, Math.round(afterGold - beforeGold)) : 0,
    streak: temp_2,
    paused: !matches && temp_2 >= temp
  };
}
...
class AutomationController {
  ...
  async queryProfile(client, input2 = !1) {
  ...
  async queryCurrentValues(client, force = !1) {
    const timestamp = Date.now();
    ...
    if (typeof client_2.queryCondition != "function" || typeof client_2.queryGold != "function" || typeof client_2.queryAttributes != "function") {
      ...
      this.conditionCache = record_2;
      this.goldCache = record;
      this.attributeCache = record_3;
      return petValues;
    }
    const temp_3 = !force && !!this.goldCache && !!(timestamp - this.goldCache.loadedAt < 120000);
    const temp_2 = !force && !!this.attributeCache && !!(timestamp - this.attributeCache.loadedAt < 600000);
    const [profile, petValues_2, temp] = await Promise.all([client_2.queryCondition(), temp_3 ? this.goldCache.value : client_2.queryGold(), temp_2 ? this.attributeCache.value : client_2.queryAttributes()]);
    const record_5 = {
      loadedAt: timestamp,
      value: profile
    };
  ...
  async queryFood(client, force = !1) {
  ...
  async queryBath(client, force = !1) {
  ...
  async queryFatigue(client, force = !1) {
  ...
  async friendCandidates() {
    const list = (await this.callAction("get_friends_with_category", {})).flatMap(temp => {
      const trimmed = String(temp.user_id ?? temp.uin ?? "").trim();
      const trimmed_2 = String(temp.remark ?? temp.nickname ?? temp.nick ?? "").trim();
  ...
  async visitCandidates(config, excludedUins = []) {
    const list = excludedUins.filter(temp => temp.isFriend ? config.visitFriends : config.visitStrangers).map(temp_2 => ({
      uin: temp_2.target.uin,
      name: temp_2.target.name,
      kind: temp_2.isFriend ? "friend" : "stranger",
      target: temp_2.target
    }));
    const temp_21 = config.visitFriends && !list.some(temp_3 => temp_3.kind === "friend");
    const temp_19 = config.visitStrangers && !list.some(temp_4 => temp_4.kind === "stranger");
    if (!temp_21 && !temp_19) {
      return [...new Map(list.map(temp_5 => [temp_5.uin, temp_5])).values()];
    }
    let list_3;
    try {
      list_3 = await this.friendCandidates();
    } catch (temp_16) {
      if (!list.length) {
        throw temp_16;
      }
      this.host.log("读取 OneBot 好友列表失败，仅使用 QQ 宠物官方候选：" + (temp_16 instanceof Error ? temp_16.message : String(temp_16)));
      return [...new Map(list.map(temp_6 => [temp_6.uin, temp_6])).values()];
    }
    const temp_22 = new Set(list_3.map(temp_7 => temp_7.uin));
    const list_4 = temp_21 ? [...list.filter(temp_8 => temp_8.kind === "friend"), ...list_3] : list.filter(temp_9 => temp_9.kind === "friend");
    const list_6 = config.visitStrangers ? list.filter(temp_10 => temp_10.kind === "stranger") : [];
    if (temp_19) {
      ...
      for (const temp_18 of filteredItems.slice(0, 3)) {
        ...
        for (const temp_17 of actionResult) {
          ...
          if (text && text !== this.host.uin && !temp_22.has(text)) {
            list_6.push({
              uin: text,
              name: trimmed,
              kind: "stranger"
    ...
    const list_5 = [...new Map(list_4.map(temp_13 => [temp_13.uin, temp_13])).values()];
    const list_2 = [...new Map(list_6.map(temp_14 => [temp_14.uin, temp_14])).values()];
    const list_7 = [];
    const temp_20 = Math.max(list_5.length, list_2.length);
    for (let number = 0; number < temp_20; number += 1) {
      if (list_5[number]) {
        list_7.push(list_5[number]);
      }
      if (list_2[number]) {
        list_7.push(list_2[number]);
      }
    }
    return list_7;
  }
  async findEmployableFriend(client, config) {
    ...
    } else if (list.some(temp_4 => temp_4.available)) {
      temp_16 = list.filter(temp_5 => temp_5.available).sort((left, right) => right.coinBonusPercent - left.coinBonusPercent || right.target.level - left.target.level).slice(0, config.workFriendScanLimit).map(temp_6 => ({
    ...
    let temp_18 = temp_16.length ? "" : flag ? "指定好友 QQ 中没有有效候选" : "好友列表没有可扫描对象";
    let catalogItem_2;
    try {
    ...
    for (const temp_15 of temp_16) {
      try {
        ...
        if (!temp_13) {
          temp_18 = this.targetLabel(temp_15) + "尚未创建宠物";
          continue;
        }
        ...
        if (!catalogItem) {
          temp_18 = this.targetLabel(temp_13) + "不能参加当前岗位";
          continue;
        }
        ...
        list_2.push(hireCandidate);
      } catch (temp_14) {
        temp_18 = this.targetLabel(temp_15) + "不可雇佣：" + (temp_14 instanceof Error ? temp_14.message : String(temp_14));
        if (temp_18.includes("不支持安卓端的好友宠物详情接口")) {
          break;
        }
      }
    }
    list_2.sort(comparePkCandidates);
    if (list_2[0]) {
      const hireCandidate_2 = list_2[0];
      this.host.log("已比较 " + list_2.length + " 只可雇佣好友宠物，选择" + this.targetLabel(hireCandidate_2.target) + "，预计雇佣加成 " + (hireCandidate_2.bonus >= 0 ? "+" : "") + hireCandidate_2.bonus);
      return hireCandidate_2;
    }
    this.host.log("未找到可雇佣的好友宠物，本次按普通打工继续" + (temp_18 ? "（" + temp_18 + "）" : ""));
    return null;
  }
  async maybeVisit(client, config, foodInventory, bathInventory) {
    ...
    try {
      ...
      for (const temp_11 of slicedItems) {
        try {
          await client.visitOther(temp_11.uin);
          this.progress.markTargetVisited(temp_11.uin);
          this.progress.increment(temp_11.kind === "friend" ? "visitFriend" : "visitStranger");
          this.host.log("自动走访" + (temp_11.kind === "friend" ? "好友" : "陌生人") + this.targetLabel(temp_11) + "成功");
          const temp_9 = config.otherCareDailyExperienceLimit > 0 && this.progress.snapshot().dailyExperienceGain >= config.otherCareDailyExperienceLimit;
          if (config.visitAutoCare && temp_9) {
            this.host.log("今日经验已达到 " + config.otherCareDailyExperienceLimit + "，停止照顾别人");
          } else if (config.visitAutoCare) {
            try {
              const temp_7 = temp_11.target ?? (await client.queryOtherPet(temp_11.uin, temp_11.kind));
              if (temp_7) {
                const petValues = await client.queryOtherValues(temp_7.petId);
                if (petValues.hunger < config.hungerThreshold && (foodInventory.biscuits ?? 0) > 0) {
                  await client.feedOther(temp_7.uin, temp_7.petId);
                  this.progress.increment("careOther");
                  this.host.log("已自动喂养走访对象");
                }
                const temp_6 = bathInventory.bathBall > 0 ? "2" : bathInventory.soap > 0 ? "1" : null;
                if (petValues.clean < config.cleanThreshold && temp_6) {
                  await client.washOther(temp_7.uin, temp_6);
                  this.progress.increment("careOther");
              ...
              this.host.log("走访已完成，暂无法读取对方宠物状态：" + (temp_8 instanceof Error ? temp_8.message : String(temp_8)));
            }
          }
          ...
          if (isMutationDeniedError(temp_10)) {
            throw temp_10;
          }
          text = temp_10 instanceof Error ? temp_10.message : String(temp_10);
        }
      }
      ...
      const temp_12 = temp_13 instanceof Error ? temp_13.message : String(temp_13);
      this.progress.setBlock("visit", temp_12, config.failureCooldownSeconds);
      this.host.log("获取走访候选失败：" + temp_12);
    }
    return !1;
  }
  ...
  async handlePendingPk(client, story) {
    const temp_3 = this.progress.snapshot().pendingPk;
    if (!temp_3) {
      return !1;
    }
    const temp_4 = Math.max(0, 8000 - (Date.now() - Date.parse(temp_3.createdAt)));
    if (temp_4 > 0) {
      this.host.updateStatus({
        activity: "PK 结果播放中，" + Math.ceil(temp_4 / 1000) + " 秒后结算；主任务不受影响"
      });
      return !1;
    }
    const combined = "settle-pk:" + temp_3.storyId;
    if (this.progress.activeBlock(combined) || (await this.blocked(story, "结算 PK"))) {
      return !1;
    }
    try {
      await client.settlePk(temp_3.storyId);
      this.progress.markStorySettled(temp_3.storyId);
      this.progress.clearPendingPk();
      this.progress.increment(temp_3.kind);
      this.progress.clearBlock(combined);
      this.host.log("PK 已结算并记录：" + temp_3.storyId);
    } catch (temp_2) {
      if (isIrrecoverableSettleError(temp_2)) {
        this.progress.dismissStory(temp_3.storyId);
        this.progress.clearPendingPk();
        this.progress.increment(temp_3.kind);
        this.progress.clearBlock(combined);
        this.host.log("PK 已由服务器结算：" + temp_3.storyId);
      } else {
  ...
  async maybePk(client, config) {
    ...
    try {
      ...
      const temp_7 = await client.startPk(temp_6);
      const temp_8 = temp_6.target.kind === "friend" ? "pkFriend" : "pkStranger";
      this.progress.setPendingPk(temp_8, temp_7.storyId, temp_6.target.uin);
      this.progress.markPkTarget(temp_6.target.uin);
      const delaySeconds_2 = randomDelaySeconds(config.pkDelayMinSeconds, config.pkDelayMaxSeconds);
      this.progress.setBlock("pk", "等待下次 PK", delaySeconds_2);
      this.host.log("已向" + (temp_6.target.kind === "friend" ? "好友" : "陌生人") + this.targetLabel(temp_6.target) + "发起 PK：我方 " + temp_6.selfPower + "，对方 " + temp_6.opponentPower + "，战力差 " + (temp_6.selfPower - temp_6.opponentPower >= 0 ? "+" : "") + (temp_6.selfPower - temp_6.opponentPower) + "；Story ID：" + temp_7.storyId);
      return !0;
    } catch (temp_9) {
  ...
  async maybeEncourageSchool(client, config, story) {
    ...
    try {
      ...
      const temp_5 = temp_6 instanceof Error ? temp_6.message : String(temp_6);
      if (/(?:已经|已).*鼓励|重复.*鼓励|鼓励.*(?:已领取|已完成)/.test(temp_5)) {
        this.progress.markStoryEncouraged(story.storyId);
      ...
      this.progress.setBlock(combined, temp_5, config.failureCooldownSeconds);
      this.host.log("学习鼓励失败：" + temp_5);
      return !1;
    }
  }
  ...
  async handleStory(client, config, story) {
    ...
    if (story.storyId) {
      ...
      if (story.finished) {
        ...
        try {
          const temp_7 = temp_11?.kind === "adventure" && config.adventureMoneyBagStopEnabled;
          let temp_6 = null;
          if (temp_7) {
            try {
              temp_6 = await this.queryGoldNow(client);
            } catch (temp_3) {
          ...
          if (temp_6 !== null && temp_8 !== null) {
            const temp_5 = updateAdventureMoneyBagStreak(this.progress.snapshot().consecutiveAdventureNoMoneyBag, temp_6, temp_8, config.adventureNoMoneyBagLimit);
          ...
          if (isIrrecoverableSettleError(temp_9)) {
            ...
            this.host.updateStatus({
              connected: !0,
              error: null,
              story: clearFinishedStoryDisplay(story, !0),
  ...
  async refreshReadonly(force = !1) {
    if (!this.busy) {
      this.busy = !0;
      try {
        const temp_4 = await this.client();
        const [profile, petValues, temp, temp_3, temp_2] = await Promise.all([this.queryProfile(temp_4, force), this.queryCurrentValues(temp_4, force), temp_4.queryStory(), this.queryFood(temp_4, force), this.queryBath(temp_4, force)]);
        const fatigue = await this.queryFatigue(temp_4, force);
        this.progress.recordAttributes(petValues);
        this.publish(profile, fatigue, petValues, temp, temp_3, temp_2);
        this.host.updateStatus({
          activity: "状态已刷新"
        });
      } finally {
  ...
  async runOnce() {
    ...
    this.busy = !0;
    try {
      const temp_22 = this.host.getConfig();
      if (!temp_22.enabled) {
        return null;
      }
      ...
      const temp_20 = await this.client();
      let [profile, petValues_2, temp_18, temp_21, temp_17] = await Promise.all([this.queryProfile(temp_20), this.queryCurrentValues(temp_20), temp_20.queryStory(), this.queryFood(temp_20), this.queryBath(temp_20)]);
      const fatigue = await this.queryFatigue(temp_20);
      this.setNextRunDelay(temp_22, temp_18);
      this.progress.recordAttributes(petValues_2);
      this.publish(profile, fatigue, petValues_2, temp_18, temp_21, temp_17);
      this.host.log("状态：金币 " + petValues_2.gold.toFixed(0) + "，心情 " + petValues_2.feel.toFixed(0) + "，体力 " + petValues_2.hunger.toFixed(0) + "，清洁 " + petValues_2.clean.toFixed(0));
      if (temp_18.finished && (await this.handleStory(temp_20, temp_22, temp_18))) {
        return "story";
      }
      const list = [];
      if (await this.handlePendingPk(temp_20, temp_22)) {
        list.push("pk_settle");
      }
      if (isRestPeriodActive(temp_22)) {
        await this.handleStory(temp_20, temp_22, temp_18);
        const temp = temp_18.storyId ? "休息时段：等待当前任务结束，剩余 " + temp_18.remainingSeconds + "s" : "休息时段 " + temp_22.restStartTime + "–" + temp_22.restEndTime + "：保持空闲";
        const statusPatch = {
          activity: temp
        };
        this.host.updateStatus(statusPatch);
      ...
      if (await this.maybeEncourageSchool(temp_20, temp_22, temp_18)) {
        list.push("encourage");
      }
      const petValues_3 = petValues_2;
      let flag = !1;
      let flag_2 = !1;
      if (temp_22.careEnabled && petValues_3.hunger < temp_22.hungerThreshold && !this.progress.activeBlock("feed") && !(await this.blocked(temp_22, "喂食"))) {
        try {
          if (temp_21.biscuits === 0) {
            if (!temp_22.autoBuySupplies) {
              this.progress.setBlock("feed", "饼干不足且自动购买已关闭", temp_22.failureCooldownSeconds);
            } else {
              await temp_20.buyFood(temp_22.foodPurchaseCount);
              temp_21 = await this.queryFood(temp_20, !0);
              if (temp_21.biscuits === 0) {
          ...
          if (temp_21.biscuits !== 0) {
            await temp_20.feed();
            flag = true;
            if (typeof temp_21.biscuits == "number" && temp_21.biscuits > 0) {
              temp_21 = {
                ...temp_21,
                biscuits: temp_21.biscuits - 1
              };
              this.foodCache = {
                loadedAt: Date.now(),
                value: temp_21
          ...
          if (isMutationDeniedError(temp_3)) {
            throw temp_3;
          }
          const temp_2 = temp_3 instanceof Error ? temp_3.message : String(temp_3);
          this.progress.setBlock("feed", temp_2, temp_22.failureCooldownSeconds);
          this.host.log("自动喂食失败，本轮继续检查其他动作：" + temp_2);
        }
      }
      if (temp_22.careEnabled && petValues_3.clean < temp_22.cleanThreshold && !this.progress.activeBlock("wash") && !(await this.blocked(temp_22, "洗澡"))) {
        try {
          let temp_4 = temp_17.bathBall > 0 ? "2" : "1";
          if ((temp_4 === "2" ? temp_17.bathBall : temp_17.soap) <= 0) {
            if (!temp_22.autoBuySupplies) {
              this.progress.setBlock("wash", "洗护用品不足且自动购买已关闭", temp_22.failureCooldownSeconds);
              temp_4 = temp_17.soap > 0 ? "1" : "2";
            } else {
              temp_4 = "2";
              if (!(await temp_20.buyBathItem(temp_4, temp_22.bathPurchaseCount)).succeeded) {
                throw new QQPetError("购买洗护用品未成功");
              }
              temp_17 = {
                ...temp_17,
                bathBall: temp_17.bathBall + temp_22.bathPurchaseCount,
                counts: {
                  ...temp_17.counts,
                  2: (temp_17.counts[2] ?? 0) + temp_22.bathPurchaseCount
                }
              };
              this.bathCache = {
                loadedAt: Date.now(),
          ...
          if ((temp_4 === "2" ? temp_17.bathBall : temp_17.soap) > 0 || temp_22.autoBuySupplies) {
            await temp_20.useBathItem(temp_4);
            flag_2 = true;
            if (temp_4 === "2" && temp_17.bathBall > 0) {
              temp_17 = {
                ...temp_17,
                bathBall: temp_17.bathBall - 1
              };
            }
            if (temp_4 === "1" && temp_17.soap > 0) {
              temp_17 = {
                ...temp_17,
                soap: temp_17.soap - 1
              };
          ...
          const temp_5 = temp_6 instanceof Error ? temp_6.message : String(temp_6);
          this.progress.setBlock("wash", temp_5, temp_22.failureCooldownSeconds);
          this.host.log("自动洗澡失败，本轮继续检查其他动作：" + temp_5);
        }
      }
      if (flag || flag_2) {
        await delaySeconds(temp_22.verifyDelaySeconds);
        try {
          const petValues = await this.queryCurrentValues(temp_20);
          petValues_2 = petValues;
          if (flag) {
            if (petValues.hunger <= petValues_3.hunger) {
              this.progress.setBlock("feed", "喂食后体力未增加", temp_22.failureCooldownSeconds);
          ...
          if (flag_2) {
            if (petValues.clean <= petValues_3.clean) {
              this.progress.setBlock("wash", "洗澡后清洁值未增加", temp_22.failureCooldownSeconds);
              this.host.log("自动洗澡结果未生效，本轮继续检查其他动作");
            } else {
              this.progress.increment("wash");
              this.progress.clearBlock("wash");
              list.push("wash");
              this.host.log("自动洗澡成功：" + petValues_3.clean.toFixed(0) + "→" + petValues.clean.toFixed(0));
            }
          }
        } catch (temp_7) {
          ...
          if (flag) {
            this.progress.setBlock("feed", temp_8, temp_22.failureCooldownSeconds);
          }
          if (flag_2) {
            this.progress.setBlock("wash", temp_8, temp_22.failureCooldownSeconds);
          }
          this.host.log("护理结果校验失败，本轮继续检查其他动作：" + temp_8);
        }
      }
      if (await this.maybePk(temp_20, temp_22)) {
        list.push("pk");
      }
      if (await this.maybeVisit(temp_20, temp_22, temp_21, temp_17)) {
        list.push("visit");
      }
      if (await this.handleStory(temp_20, temp_22, temp_18)) {
        if (list.length) {
      ...
      const temp_19 = resolveTaskForFatigue(temp_22, fatigue, this.decide(temp_22, petValues_2));
      if (temp_19 === "rest") {
      ...
      if (await this.blocked(temp_22, temp_19 === "school" ? "学习" : temp_19 === "work" ? "打工" : "冒险")) {
        if (list.length) {
          return list.join("+");
        } else {
          return temp_19;
      ...
      if (temp_19 === "school") {
        const attribute = rotateSchoolAttribute(temp_22, this.progress.snapshot().schoolRotationIndex);
        const temp_11 = temp_22.schoolRotationEnabled ? 0 : temp_22.courseSubEvent;
        const temp_10 = await temp_20.startSchool(attribute, temp_11);
        this.progress.setPending("school", temp_10.storyId);
        const member = {
          physical: "力量",
          culture: "智力",
          art: "魅力"
        }[attribute];
        this.host.log("已开始" + member + "学习“" + temp_10.item.name + "”" + (temp_10.storyId ? "，storyId=" + temp_10.storyId : ""));
      } else if (temp_19 === "work") {
        const hireCandidate = temp_22.employFriend ? await this.findEmployableFriend(temp_20, temp_22) : null;
        const temp_12 = await temp_20.startWork(temp_22.workCareerType, hireCandidate?.jobSubEvent ?? temp_22.workJobSubEvent, hireCandidate ? {
          uin: hireCandidate.target.uin,
          petId: hireCandidate.target.petId
        } : undefined);
        this.progress.setPending("work", temp_12.storyId);
        const temp_13 = temp_12.hiredFriend && hireCandidate ? "，已雇佣好友" + this.targetLabel(hireCandidate.target) + "（加成 " + (hireCandidate.bonus >= 0 ? "+" : "") + hireCandidate.bonus + "）" : "";
        this.host.log("已开始打工“" + temp_12.item.name + "”" + temp_13 + (temp_12.storyId ? "，storyId=" + temp_12.storyId : ""));
      } else {
        let temp_15;
        try {
          temp_15 = await temp_20.startAdventure("", temp_22.quickAdventureEnabled, temp_22.adventurePreviewRefreshLimit);
        } catch (temp_14) {
          if (temp_14 instanceof QQPetError && temp_14.code === "coin_bag_not_found") {
            this.nextRunDelaySeconds = Math.max(5, temp_22.intervalSeconds);
        ...
        const temp_16 = temp_22.quickAdventureEnabled ? "（刷新 " + temp_15.refreshCount + " 次命中钱袋子）" : "";
