...
if(embedded){
  ...
  document.body.innerHTML=`<main><section class="panel" style="max-width:460px;margin:12vh auto;text-align:center"><h1>Nico QQ 宠物</h1><p style="margin:14px 0">${opened?'已在独立标签页打开。':'浏览器阻止了自动打开，请点击下面的按钮。'}</p><a href="${safeUrl}" target="qqpet-dashboard" style="display:inline-block;border-radius:8px;background:#6857e5;color:white;padding:8px 14px;text-decoration:none;font-weight:600">在独立标签页打开</a></section></main>`;
}else{
